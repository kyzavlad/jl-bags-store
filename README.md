...
### Разбиение по материалам

Если после кода идёт маркер материала/группы (`Е`/`E`, `З`, `Р`, а также сочетания
вроде `ЗР`), товар разбивается на **отдельные карточки** по `код-маркер`:

| Строка в таблице | Карточка товара | Вариант (цвет) |
| --- | --- | --- |
| `5304 Е бордо` | `5304-Е` | бордо |
| `5304 Е чорний` | `5304-Е` | чорний |
| `5304 З беж` | `5304-З` | беж |
| `5304 Р бордо` | `5304-Р` | бордо |

Так каждая группа материала (`5304-Е`, `5304-З`, `5304-Р`) — это отдельная
карточка со своими ценой, описанием, материалом, размером, категорией и фото.
Поиск по `5304` в админке покажет все три карточки.

Если маркера материала нет (например `1234 чорний`), товар группируется по
числовому коду как раньше: карточка `1234`, вариант `чорний`.
